...
def register():
    if request.method == "POST":
        ...
        data = load_users()
        data["users"].append({"username": username, "password_hash": pw_hash})
        save_users(data)
...
def add_category():
    if request.method == "POST":
        ...
        inv["categories"].append({"id": new_id, "name": name, "models": []})
        save_inventory(inv)
...
def add_model(cat_id):
    ...
    if request.method == "POST":
        ...
        save_inventory(inv)
        flash(
            f"Model '{model_name}' added under category '{category['name']}'.",
            "success",
        )
        return redirect(url_for("inventory"))

    return render_template(
        "add_model.html", title="Add Model", category=category, mod_id=model_id
    )
...
def update_quantity(cat_id, model_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return render_template(
                "update_quantity.html", category=category, model=model
            )
...
def updates_history():
    ...
    inv = load_inventory()
    return render_template(
        "updates_history.html", title="Updates History", updates=updates, inv=inv
    )
...
def operation_history():
    ...
    # Else render page
    return render_template(
        "operation_history.html", history=filtered, departments=dep, inv=inv
    )
