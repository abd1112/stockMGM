...
# ------------------------------ give item to ----------------------
@app.route("/give_item_to", methods=["GET", "POST"])
@login_required
def give_item_to():
    inv = load_inventory()
    dep = load_dep()
    ...
    if request.method == "POST":
        ...
        for depart in dep["departments"]:
            if depart["name"] == department:
                depart["history"].insert(0,f"{qty} {item} {model}")
    ...
    return render_template("give_item_to.html", title="Give an Item To", inventory=inv, departments=dep)
# ------------------------------ departments ----------------------
...
def operation_history():
    ...
    return render_template("operation_history.html", history=history, departments=dep)
